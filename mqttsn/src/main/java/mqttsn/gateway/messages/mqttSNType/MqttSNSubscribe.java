 ...
 */
public class MqttSNSubscribe extends MqttSNMessage {
    //MqttSN SUBSCRIBE message
     ...
     */
    public MqttSNSubscribe(byte[] data) throws MqttSNException {
        msgType = SUBSCRIBE;
        dup = ((data[2] & 0x80) >> 7 != 0);
        qos = (data[2] & 0x60) >> 5;
        if(qos == 4) qos = -1;
